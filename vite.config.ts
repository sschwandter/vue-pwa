...
export default defineConfig(({ command }) => {
  // The GitHub Pages project site is served from /<repo>/, so production builds
  // use that base path; the dev server stays at root. The manifest
  // start_url/scope derive from it so the installed PWA's scope matches.
  // (Hosting at a root domain instead? Set this to "/".)
  const base = command === "build" ? "/vue-pwa/" : "/";
